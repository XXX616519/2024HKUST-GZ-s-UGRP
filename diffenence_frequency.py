...
# 计算频率差分（答对频率 - 答错频率）
mlp_diff_freq = {k: mlp_match_freq.get(k, 0) - mlp_non_match_freq.get(k, 0) for k in set(mlp_match_freq) | set(mlp_non_match_freq)}
attn_diff_freq = {k: attn_match_freq.get(k, 0) - attn_non_match_freq.get(k, 0) for k in set(attn_match_freq) | set(attn_non_match_freq)}

# 绘图函数（差分图）
def plot_diff_counts(counter, title):
    # 过滤出差分大于等于 0.01 的条目
    filtered_counter = {k: v for k, v in counter.items() if abs(v) >= 0.01}
    ...
    plt.title(title)
    plt.xlabel('Type')
    plt.ylabel('Frequency Difference (Match - Non-Match)')
    plt.tight_layout()  
    plt.show()

# 绘制答对与答错的分布差分图
plot_diff_counts(mlp_diff_freq, "MLP Frequency Difference (Match - Non-Match)")
plot_diff_counts(attn_diff_freq, "Attention Frequency Difference (Match - Non-Match)")
